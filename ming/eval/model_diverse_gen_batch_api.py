    ...
    while response is None:
        try:
            response = client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": "You are a helpful assistant. Always think twice before giving your response."},
...
def call_api(api_type, query, args):
    api = {"chatgpt": chatgpt_api}[api_type]
    result = api(query, args)
    return result

# Custom dataset class
class CustomDataset:
    def __init__(self, questions, batch_size, task_specific_prompt):
        self.questions = questions
        self.batch_size = batch_size
        self.size = len(questions)
        self.task_specific_prompt = task_specific_prompt
        self.template = """你是一个精通医学知识的专家。以下是一个关于医学知识的问题，请一步一步地思考这个问题，并在最后用"答案为{{answer_idx}}"给出你的答案。
    ...
    def __getitem__(self, index):
        ...
        for i in range(index*bz, (index+1)*bz):
            if i < self.size:
                ...
                prompts.append(self.template.format(question=question))
...
def eval_model(args):
    ...
    #     task_specific_prompt = "\n\n请用选项的字母直接回答，不要输出其他信息："
        ...
        # task_specific_prompt = "\n\nPlease answer with option letter directly, do not output other infomation."
    dataset = CustomDataset(questions, batch_size=args.batch_size, task_specific_prompt=task_specific_prompt)
    for idx in trange(len(dataset)):
        questions, prompts, answers, additional_infos = dataset[idx]
        if len(questions) == 0:
            break
        
        output = call_api(args.api_name, questions[0], args)
        outputs = [output]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--api-name", type=str, default="chatgpt")
