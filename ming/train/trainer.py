...
class MINGTrainer(Trainer):
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        #     keys_to_match = ['mlp.gate_proj.experts', "mlp.up_proj.experts", "mlp.down_proj.experts"]
        #     if getattr(self.args, "use_im_start_end", False):
        #         keys_to_match.extend(['embed_tokens', 'embed_in'])
        ...
        #     self.args.distributed_state.wait_for_everyone()
            ...
            run_dir = self._get_output_dir(trial=trial)
            output_dir = os.path.join(run_dir, checkpoint_folder)
            keys_to_match = ["mlp.gate_proj.experts", "mlp.up_proj.experts", "mlp.down_proj.experts", "mlp.switch"]

            weight_to_save = get_molora_state_maybe_zero_3(self.model.named_parameters(), keys_to_match)

            if self.args.local_rank == 0 or self.args.local_rank == -1:
                self.model.config.save_pretrained(output_dir)
                torch.save(weight_to_save, os.path.join(output_dir, f'molora.bin'))
