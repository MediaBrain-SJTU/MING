...
def attention_fn(
    ...
    if layer_past is not None:
        ...
        key_layer = torch.cat((past_key.to(key_layer.device), key_layer), dim=0)
        value_layer = torch.cat((past_value.to(value_layer.device), value_layer), dim=0)
...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    def get_masks(self, seq, device):
        q_context_length = seq.index(self.config.bos_token_id) + 1

        attention_mask = torch.ones((1, len(seq), len(seq)), device=device)
        attention_mask.tril_()
        attention_mask[..., :q_context_length - 1] = 1
        if seq.count(20003) != 0:
            pad_start = seq.index(20003)
            pad_end = seq.index(self.config.bos_token_id) - 1
            attention_mask[...,  pad_start:pad_end] = 0
    ...
    def get_position_ids(self, seq, mask_position, device, gmask=False):
        #context_length = seq.index(self.config.eos_token_id) + 1
    ...
    def forward(
            ...
            for i in range(input_ids.size(0)):
                ...
                if self.training is True:
                    if attention_mask is None:
                        ...
                        attention_mask1 = self.get_masks(
                            seq=seq,
                            device=input_ids.device
                        )
                        attention_mask = torch.cat([attention_mask,attention_mask1])
                    ...
                    if position_ids is None:
                        ...
                        mask_position = seq.index(mask_token) 
                        position_ids1 = self.get_position_ids(
                            seq=seq,
                            mask_position=mask_position,
                            device=input_ids.device,
                            gmask=use_gmask
                        )
                        position_ids = torch.cat([position_ids,position_ids1])
                else:
                    if attention_mask is None:
                        attention_mask = self.get_masks(
                            seq=seq,
        ...
        for i, layer in enumerate(self.layers):
            ...
            layer_ret = layer(
                hidden_states.to(layer.input_layernorm.weight.device),
                position_ids=position_ids.to(layer.input_layernorm.weight.device),
                attention_mask=attention_mask.to(layer.input_layernorm.weight.device),
        ...
        # Final layer norm.
        hidden_states = self.final_layernorm(hidden_states.to(self.final_layernorm.weight.device))
...
class ChatGLMForConditionalGeneration(ChatGLMPreTrainedModel):
    ...
    def forward(
        ...
        hidden_states = transformer_outputs[0].to(self.lm_head.weight.device)
